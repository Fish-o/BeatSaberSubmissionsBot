...
async function command(client, member, teamRoleId, opponentRoleId, homeScore, opponentScore){
    if(!homeScore || isNaN(homeScore) || !opponentScore || isNaN(opponentScore)){
        return await errorembed('The entered score is invalid');
    }
    if(!member.roles.cache.has(teamRoleId) || member.roles.cache.has(opponentRoleId)){
        return await errorembed('You arent allowed to submit this score')
    }
    let dbres = await SubmissionsModel.find({ $or:[
    ...
    let opponent_submission = dbres?.find(doc => doc.teamId == opponentRoleId);
    let desc = ``;
    
    let matching;
    if(opponent_submission){
        if(opponent_submission.homeScore !== opponentScore || opponent_submission.awayScore !== homeScore){
...
`❌ Wrong scores found: (${guild.roles.cache.get(teamRoleId)} - ${guild.roles.cache.get(opponentRoleId)})
${guild.roles.cache.get(teamRoleId)}: ${homeScore}-${opponentScore}
${guild.roles.cache.get(opponentRoleId)}: ${opponent_submission.awayScore}-${opponent_submission.homeScore}`)
                matching = false;
                await SubmissionsModel.updateOne({teamId:opponentRoleId, opponentId:teamRoleId}, {matching:false})
        } else{
            matching = true;
            await SubmissionsModel.updateOne({teamId:opponentRoleId, opponentId:teamRoleId}, {matching:true})
        }
    ...
    if(old_submission){
        if(opponent_submission){
            if(opponent_submission.homeScore !== old_submission.awayScore || opponent_submission.awayScore !== old_submission.homeScore){
                if(matching === true){
                    guild.channels.cache.get(notifyChannel).send(
`✔️ Scores are now corrected: (${guild.roles.cache.get(teamRoleId)} - ${guild.roles.cache.get(opponentRoleId)})`)
                }
            } 
        }
        await SubmissionsModel.updateOne({teamId:teamRoleId, opponentId:opponentRoleId},{
            memberid:member.id,
            teamId:teamRoleId,
        ...
        desc += `Updated score, old data: \`${old_submission.memberid}-${old_submission.teamId}-${old_submission.opponentId} ${old_submission.homeScore}-${old_submission.awayScore} ${old_submission.timestamp.toLocaleString("en-US", {timeZone: 'America/New_York'})} ${old_submission.matching}\`\n\n`
    }
    else{
    ...
    return new Discord.MessageEmbed().setTitle('Saved the results!').setTimestamp().setColor('GREEN').setDescription(
desc+`Member: ${member}
...
exports.interaction = async(client, interaction, args)=>{
    ...
    let res = await command(client, interaction.member, yourTeam, opponentTeam, yourGoals, opponentGoals);
